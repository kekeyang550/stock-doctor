...
8. 运行配置可视化
   ...
   - Tushare 财务指标已扩展到 EPS、毛利率、资产负债率；诊断证据会在字段存在时解释毛利空间和财务杠杆，数据质量会校验异常比例。
...
1. 财务数据第二阶段
   - 已完成 Tushare Pro 包/token 准备度展示、安全 provider 回退、基础资料、基础财务指标、EPS、毛利率、资产负债率和前复权日线读取。
   - 下一步在 token 可用后做真实 token 实盘连通性校验，并继续扩展现金流、负债结构等更细财报字段。
