...
5. 策略回测数据血缘
   ...
   - 回测报告已增加退出原因分布，前端面板和 HTML/Markdown 报告可汇总持有到期、止盈、止损、跌破 MA20、缩量退出的笔数。
...
3. 组合风险和回测真实化
   ...
   - 回测策略已增加止盈/止损、MA20 跌破和量比低于阈值退出规则，并保存每次参数组合；下一步可继续扩展诊断分变化触发的买卖规则。
