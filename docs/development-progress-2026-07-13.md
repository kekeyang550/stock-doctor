...
5. 策略回测数据血缘
   ...
   - 2026-07-16 回测报告已增加诊断快照覆盖率统计，显示诊断转弱退出中使用已保存诊断快照的交易数、代理分回退交易数和覆盖百分比；前端面板、HTML 报告和 Markdown 报告均展示该口径。
...
- 后端全量测试：`193 passed, 1 warning`（测试时临时使用 `STOCK_DOCTOR_DATA_PROVIDER=mock`，避免本机真实 provider 网络波动影响单元测试）
- 前端测试：`57 passed`
- 前端生产构建：通过
...
- 2026-07-16 诊断快照覆盖率验证：后端回测/API 定向测试 `20 passed, 1 warning`，前端测试 `57 passed`。
