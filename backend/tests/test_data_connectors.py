...
def test_runtime_config_endpoint_exposes_provider_and_local_paths():
    ...
    assert payload["auto_refresh"]["enabled"] == settings.data_auto_refresh_enabled
    assert payload["auto_refresh"]["interval_minutes"] == settings.data_auto_refresh_interval_minutes
    assert payload["auto_refresh"]["scope"] == settings.data_auto_refresh_scope
    assert payload["auto_refresh"]["run_on_startup"] == settings.data_auto_refresh_on_startup
    assert payload["auto_refresh"]["running"] is False
    assert payload["auto_refresh"]["run_count"] >= 0
    assert {"next_run_at", "last_run_status", "last_error"}.issubset(payload["auto_refresh"].keys())
