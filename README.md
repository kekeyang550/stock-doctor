...
If the Tushare or TongDaXin read-only probes have been run in the runtime panel, JSON/HTML/Markdown exports also include their status, warning details, candidate paths or steps, and the recommended next action. If a probe has not been run, exports explicitly say that the corresponding probe was not executed for this report.
