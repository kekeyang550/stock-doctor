  ...
  trades: [
    {
      ...
      diagnosis_exit_note: '历史诊断快照基线 86 校准后路径分 82.3 未低于阈值 65。',
      diagnosis_exit_source: 'historical-snapshot',
      diagnosis_exit_baseline_score: 86,
      diagnosis_exit_baseline_date: '2026-07-01T06:00:00Z',
      rule_tags: ['综合高分', '技术强势'],
...
describe('App', () => {
  ...
  it('imports portfolio lots with balance and diluted cost headers', async () => {
    render(<App />)

    const input = await screen.findByLabelText('导入持仓文件')
    const file = new File(['证券代码\t证券名称\t证券余额\t摊薄成本价\n600519\t贵州茅台\t16\t998.6\n'], 'broker-balance.txt', {
      type: 'text/plain',
    })
    fireEvent.change(input, { target: { files: [file] } })

    await waitFor(() => {
      expect(screen.getByLabelText('持仓数量 贵州茅台')).toHaveValue(16)
      expect(screen.getByLabelText('成本价 贵州茅台')).toHaveValue(998.6)
    })
    await waitFor(() => {
      const portfolioCalls = vi.mocked(fetch).mock.calls
        .map((call) => decodeURIComponent(String(call[0])))
        .filter((url) => url.includes('/risk/portfolio'))
      expect(portfolioCalls.some((url) => url.includes('holdings=600519:16:998.6'))).toBe(true)
    })
  })
  ...
  it('imports broker trades with trade flag and average price headers', async () => {
    render(<App />)

    const input = await screen.findByLabelText('导入交易流水文件')
    const file = new File([
      '成交日期\t证券代码\t证券名称\t买卖标志\t成交股数\t成交均价\n',
      '20260710\t600519\t贵州茅台\t买入成交\t20\t1000\n',
      '20260711\t600519\t贵州茅台\t卖出成交\t5\t1030\n',
    ], 'broker-trade-flags.txt', { type: 'text/plain' })
    fireEvent.change(input, { target: { files: [file] } })
  ...
  it('shows strategy backtest summary with sample trades and drawdown', async () => {
    ...
    expect(within(backtestPanel).getByText('诊断分说明：历史诊断快照基线 86 校准后路径分 82.3 未低于阈值 65。')).toBeInTheDocument()
  ...
  it('reloads strategy backtest when fee, slippage, sample limit, MA20, volume, and diagnosis exits change', async () => {
    render(<App />)
    ...
    const diagnosisInput = within(backtestPanel).getByLabelText('回测诊断退出分')
    ...
    fireEvent.change(diagnosisInput, { target: { value: '65' } })
    ...
    await waitFor(() => {
      ...
      expect(backtestCalls.some((url) => url.includes('diagnosis_exit_score=65'))).toBe(true)
    })
  })
  ...
  it('uses saved strategy backtest parameters on first load', async () => {
    ...
    expect(within(backtestPanel).getByLabelText('回测诊断退出分')).toHaveValue(65)
  ...
  it('saves changed strategy backtest parameters to local storage', async () => {
    ...
    fireEvent.change(within(backtestPanel).getByLabelText('回测诊断退出分'), { target: { value: '65' } })
  ...
  it('falls back to default strategy backtest parameters when local storage is invalid', async () => {
    ...
    expect(within(backtestPanel).getByLabelText('回测诊断退出分')).toHaveValue(0)
  ...
  it('exports saved report archives as HTML and Markdown', async () => {
    ...
    expect(markdown).toContain('- 周期: 中线')
  ...
  it('exports a readable HTML research report from the current v2 payload', async () => {
    ...
    expect(html).toContain('波段')
    expect(html).toContain('结论页')
    expect(html).toContain('核心结论')
    expect(html).toContain('固定风险提示')
    expect(html).toContain('不构成任何投资建议或收益承诺')
    ...
    expect(html).toContain('持仓输入')
    expect(html).toContain('symbol,shares,cost_price')
    ...
    expect(html).toContain('历史诊断快照基线 86 校准后路径分 82.3 未低于阈值 65。')
    ...
    expect(html).toContain('数据质量 90 分')
    expect(html).toContain('数据质量检查')
    expect(html).toContain('来源覆盖 · 需核验')
    expect(html).toContain('真实来源：东方财富估值详情、通达信本地 K 线；保守估算：成长字段。')
    expect(html).toContain('运行配置')
    expect(html).toContain('当前数据源')
  ...
  it('exports a readable Markdown research report from the current v2 payload', async () => {
    ...
    expect(markdown).toContain('# 贵州茅台 研究报告')
    expect(markdown).toContain('- 周期: 波段')
    expect(markdown).toContain('## 结论页')
    expect(markdown).toContain('核心结论')
    expect(markdown).toContain('## 固定风险提示')
    expect(markdown).toContain('不构成任何投资建议或收益承诺')
    ...
    expect(markdown).toContain('持仓输入')
    ...
    expect(markdown).toContain('## 数据可信度')
    expect(markdown).toContain('数据质量: 90 分 / 需核验 / 覆盖 85.7%')
    expect(markdown).toContain('质量摘要: 数据质量 90 分')
    expect(markdown).toContain('### 数据质量检查')
